...
class StoryCreate(BaseModel):
    ...
    media_type: str = "image"  # "image" | "video" | "live"
...
def _to_response(s: Story, viewer_id: int, db: Session) -> StoryResponse:
    ...
    is_news = bool(getattr(s.author, "is_news_channel", 0)) if s.author else False
    return StoryResponse(
        ...
        is_news_channel=is_news,
        live_url=getattr(s, "live_url", None),
...
def create_story(
    ...
    # Only verified news channels can create live stories
    if media_type == "live" and not getattr(current_user, "is_news_channel", False):
        raise HTTPException(status_code=403, detail="البث المباشر متاح فقط للقنوات الإخبارية الموثقة")
    ...
    story = Story(
        ...
        live_url=body.live_url if media_type == "live" else None,
        expires_at=expires,
    )
    db.add(story)
    db.commit()
